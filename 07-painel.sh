#!/bin/bash
# ─── VibeCarros — Script 07: Painel do Usuário ───────────────────────────────
# Execute em: ~/vibecarros/vibecarros
# Comando: bash ~/Downloads/07-painel.sh

set -e
mkdir -p app/painel
mkdir -p app/api/painel/anuncios

# ── API: anúncios do usuário logado ───────────────────────────────────────────
cat > app/api/painel/anuncios/route.ts << 'EOF'
...
export async function GET() {
  ...
  // Auto-expiração: marca INATIVO se passou dos 60 dias
  const vencidos = anuncios.filter(a => a.status === "ATIVO" && a.expiresAt < new Date())
  if (vencidos.length > 0) {
    await prisma.anuncio.updateMany({
      where: { id: { in: vencidos.map(a => a.id) } },
      data: { status: "INATIVO" },
    })
    vencidos.forEach(a => { a.status = "INATIVO" })
  }

  const db = await prisma.usuario.findUnique({
    where: { id: usuario.id },
    select: { tipoDoc: true, planType: true },
  })

  return NextResponse.json({ anuncios, tipoDoc: db?.tipoDoc, planType: db?.planType })
}

// PATCH: renovar, inativar ou marcar como vendido
export async function PATCH(req: NextRequest) {
  const usuario = getUsuarioLogado()
  if (!usuario) return NextResponse.json({ error: "Não autorizado." }, { status: 401 })

  const { id, acao } = await req.json() // acao: "renovar" | "inativar" | "vendido" | "deletar"
  ...
  if (acao === "renovar") {
    ...
    novaExpiracao.setDate(novaExpiracao.getDate() + 60)
    await prisma.anuncio.update({
      where: { id },
      data: { status: "ATIVO", expiresAt: novaExpiracao, renovadoEm: new Date() },
    })
    return NextResponse.json({ ok: true })
  }

  const statusMap: Record<string, "ATIVO" | "INATIVO" | "VENDIDO"> = {
    inativar: "INATIVO",
    vendido: "VENDIDO",
  }
  if (statusMap[acao]) {
    await prisma.anuncio.update({ where: { id }, data: { status: statusMap[acao] } })
    return NextResponse.json({ ok: true })
  }
...
const STATUS_LABEL: Record<string, string> = { ATIVO: "✅ Ativo", INATIVO: "⏸️ Inativo", VENDIDO: "🏁 Vendido" }
const STATUS_COLOR: Record<string, string> = { ATIVO: "#22c55e", INATIVO: "#888", VENDIDO: "#f5a623" }

interface Anuncio {
  id: string; marca: string; modelo: string; anoMod: number; preco: number
  status: string; expiresAt: string; fotos: { url: string }[]
}
            ...
            <div key={a.id} style={s.card}>
              ...
              <div style={s.cardInfo}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
                  ...
                  <span style={{ ...s.badge, color: STATUS_COLOR[a.status] }}>{STATUS_LABEL[a.status]}</span>
                </div>
                <p style={s.preco}>R$ {Number(a.preco).toLocaleString("pt-BR")}</p>
                <p style={s.expira}>
                  {a.status === "ATIVO"
                    ? `Expira em: ${new Date(a.expiresAt).toLocaleDateString("pt-BR")}`
                    : a.status === "INATIVO" ? "Expirado — clique em Renovar para reativar" : ""}
                </p>
              </div>
              <div style={s.cardAcoes}>
                {a.status === "INATIVO" && (
                  <button style={s.btnAcao} onClick={() => acao(a.id, "renovar")}>🔄 Renovar</button>
                )}
                {a.status === "ATIVO" && (
                  <button style={s.btnAcao} onClick={() => acao(a.id, "inativar")}>⏸ Pausar</button>
                )}
                {a.status !== "VENDIDO" && (
                  <button style={s.btnAcao} onClick={() => acao(a.id, "vendido")}>🏁 Vendido</button>
                )}
...
echo "✅ Painel criado! /painel com gestão completa de anúncios."
echo ""
echo "🎉 Setup completo! Ordem de execução:"
echo "   1. bash 01-schema-prisma.sh"
echo "   2. bash 02-env-migrate.sh     ← edite o .env com DATABASE_URL do Neon"
echo "   3. bash 03-lib-auth.sh"
echo "   4. bash 04-api-routes.sh"
echo "   5. bash 05-middleware.sh"
echo "   6. bash 06-paginas-auth.sh"
echo "   7. bash 07-painel.sh"
echo "   8. npm run dev"
